...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        """)
        corner_label.resize(42, 37)  # Размер метки
    ...
    def add_table_row(self):
        ...
        try:
            ...
            for i, (name, type_) in enumerate(zip(column_names, column_types)):
                ...
                elif name.lower() in ['created_at']:
                    # Для временных меток используем текущее время
                    default_values[i] = str(datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    ...
    def save_new_rows(self, table_name):
        """Сохранение новых строк в базу данных с записью даты в последний столбец"""
        if table_name not in self.new_rows or not self.new_rows[table_name]:
            return

        try:
            conn = sqlite3.connect('../dist/school_schedule.db')
            cursor = conn.cursor()

            # Получаем информацию о столбцах таблицы
            cursor.execute(f"PRAGMA table_info({table_name})")
            columns = [col[1] for col in cursor.fetchall() if col[1].lower() != 'created_at']

            # Получаем индекс столбца с датой (последний столбец)
            date_column = self.table.columnCount() - 1
            ...
            for row in self.new_rows[table_name]:
                ...
                # Формируем данные для вставки (все столбцы кроме последнего)
                row_data = [
                ...
                columns_str = ", ".join(columns + ['created_at'])
                placeholders = ", ".join(["?"] * len(columns) + ["datetime('now')"])

                query = f"INSERT INTO {table_name} ({columns_str}) VALUES ({placeholders})"
                cursor.execute(query, row_data)

                # Получаем только что добавленную запись с датой
                cursor.execute(
                    f"SELECT datetime(created_at, 'localtime') FROM {table_name} WHERE rowid = last_insert_rowid()")
                created_date = cursor.fetchone()[0]

                # Обновляем ячейку с датой в таблице
                date_item = QTableWidgetItem(created_date)
                date_item.setFlags(Qt.ItemFlag.ItemIsEnabled)  # Только для чтения
                self.table.setItem(row, date_column, date_item)
        ...
        try:
            ...
            for row_idx, row in enumerate(data):
                ...
                for col_idx, value in enumerate(row):
                    ...
                    if row_idx % 2 == 0:
                        item.setBackground(QColor(27, 27, 27))
                    else:
                        item.setBackground(QColor(35, 35, 35))
