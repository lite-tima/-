# РАБОТА С ГЛОБАЛЬНЫМ РАСПИСАНИЕМ
import sys
import sqlite3
...
class ScheduleItemDelegate(QStyledItemDelegate):
    ...
    def setModelData(self, editor, model, index):
        ...
        if not text:
            ...
            model.setData(index, QColor(127, 111, 102), Qt.ItemDataRole.BackgroundRole)  # Темно-бежевый
            model.setData(index, QColor(255, 255, 255), Qt.ItemDataRole.ForegroundRole)  # Белый текст
            self.delete_schedule_record(index)
            self.parent().check_teacher_conflicts()  # Обновляем конфликты
            return
        ...
        model.setData(index, QColor(255, 255, 255), Qt.ItemDataRole.ForegroundRole)  # Белый текст
        model.setData(index, full_data, Qt.ItemDataRole.UserRole)
        self.parent().check_teacher_conflicts()  # Проверяем конфликты
...
class ScheduleApp(QMainWindow):
    ...
    def check_teacher_conflicts(self):
        """Проверка конфликтов с цветовой подсветкой"""
        # Сбрасываем все цвета на стандартные
        for row in range(self.table.rowCount()):
            for col in range(1, self.table.columnCount()):
                item = self.table.item(row, col)
                if item:
                    item.setBackground(QColor(127, 111, 102))  # Темно-бежевый фон
